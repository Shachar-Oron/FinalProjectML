...
# לא טוב צריך לשנות את זה למה שדניאל עשתה
data <- read.csv("../FinalProjectML/imputed_data.csv")
...
if (anyNA(train_data) || anyNA(test_data)) {
  stop("Missing values found in the data. Please handle or remove them before applying KNN.")
}
...
for (K in 2:20) {
  predictions <- knn(train = train_data,
                     test = test_data, cl = train_labels, k = K)
...
# Plot the ROC curve
plot.roc(test_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve., KNN_DF$'8', main = "KNN ROC", print.auc = TRUE)
...
# SVM with kernel = "vanilladot"
MS_classifier_vanilladot <- ksvm(Diagnosed.with.endo..ie.at.least.one.biopsy.positve. ~ .,
                                data = train_data, kernel = "vanilladot", prob.model = TRUE, type = "C-svc")
ksvmPredict_vanilladot <- predict(MS_classifier_vanilladot, test_data)
table(ksvmPredict_vanilladot, test_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve.)

# SVM with kernel = "anovadot"
MS_classifier_anovadot <- ksvm(Diagnosed.with.endo..ie.at.least.one.biopsy.positve. ~ .,
                              data = train_data, kernel = "anovadot", prob.model = TRUE, type = "C-svc")
ksvmPredict_anovadot <- predict(MS_classifier_anovadot, test_data)
table(ksvmPredict_anovadot, test_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve.)

# Calculate AUC for KSVM with kernel = "anovadot"
ksvm_auc <- auc(as.numeric(test_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve.), as.numeric(ksvmPredict_anovadot))
print(paste("AUC for KSVM (anovadot): ", toString(ksvm_auc), sep = ""))

# Plot ROC curve for KSVM
plot.roc(test_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve., as.numeric(ksvmPredict_anovadot),
         main = "KSVM ROC", print.auc = TRUE)

```
ROC use the ratio between Sensitivity(0.7317), Specificity(0.857).

```{r}
# SVM Confusion Matrix
svmConfMat
```
Accuracy of 0.8. This is good compared to the fact that our data is small.Bonus, the Confusion Matrix is looking good. 
### Improving SVM Model
we will try another method of svm and lets see if we are capable of improving the results.  
```{r}
train_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve. <- as.factor(train_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve.)
zero_var <- nearZeroVar(train_data, saveMetrics = TRUE)

fitControl <- trainControl(
  method = "repeatedcv",
  repeats = 5,
  classProbs = TRUE,
  summaryFunction = twoClassSummary,  # Use ROC as the summary function for binary classification
  allowParallel = TRUE
)
...
# Define the training control for classification
fitControl <- trainControl(
  method = "repeatedcv",
  repeats = 5,
  classProbs = TRUE,
...
levels(train_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve.)
# Output: [1] "0" "1"

# Convert the outcome variable to a factor with two levels
train_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve. <- factor(train_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve., levels = c("0", "1"))
...
svmPredictionProb <- predict(svmModel, test_data, probability = TRUE)

# Extract the probabilities for the positive class
svmPredictionProb <- attr(svmPredictionProb, "probabilities")[, 2]

# Convert the predicted values to a factor with explicit levels
svmPrediction <- factor(svmPrediction, levels = c("0", "1"))
test_label <- factor(test_data$Diagnosed.with.endo..ie.at.least.one.biopsy.positve., levels = c("0", "1"))

# Evaluate the model performance
svmConfMat <- confusionMatrix(svmPrediction, test_label)
...
In our case the data  is not linearly separable, however, the cost argument takes on real importance. This quantifies the penalty associated with having an observation on the wrong side of the classification boundary.
this plot shows our data in higher dimension so it can be separable. 
```{r}
# Generate some sample data
set.seed(123)
x <- matrix(rnorm(100*2), ncol = 2)
y <- factor(ifelse(x[,1] + x[,2] > 0, 1, -1))
dat <- data.frame(x = x, y = y)

# Perform PCA for dimensionality reduction
pca <- prcomp(dat[, -ncol(dat)], center = TRUE, scale. = TRUE)

# Project the data onto the first two principal components
projected_data <- as.data.frame(predict(pca, newdata = dat[, -ncol(dat)]))

# Combine the projected data with the labels
projected_data <- cbind(projected_data, y = dat$y)

# Plot the projected data in a higher dimension
ggplot(data = projected_data, aes(x = PC1, y = PC2, color = y)) +
  geom_point(size = 3) +
  labs(color = "Class") +
  theme_minimal() +
  xlab("Principal Component 1") +
  ylab("Principal Component 2") +
  ggtitle("Data in Higher Dimension (PCA Projection)")
...
# Plot the original data in a higher dimension
ggplot(data = dat, aes(x = x[, 1], y = x[, 2], color = y)) +
  geom_point(size = 3) +
  labs(color = "Class") +
  theme_minimal() +
  xlab("X1") +
  ylab("X2") +
  ggtitle("Data in Higher Dimension")


```

The higher the value of cost, the more the model tries to avoid a mis-classification.
We want to decide what is the "best" cost for our data. So  instead of specifying a cost up front, we used the tune() function from e1071 to test various costs and identify which value produces the best fitting model:
